...
static int __init smemc_init(void)
{
	if (cpu_is_pxa3xx()) {
		/*
		 * The only documentation we have on the Register (CSMSADRCFG) is that
		 * it must be programmed to 0x2. Moreover, in the bit definitions, the
		 * second bit (CSMSADRCFG[1]) is called "SETALWAYS".
		 */
		__raw_writel(0x2, CSMSADRCFG);
